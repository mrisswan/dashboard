...
app.get("/max", async (req, res) => {
  conn.query(
    "SELECT menu.id_menu, menu.nama_menu, MAX(detail_transaksi.qty) AS total_qty FROM menu JOIN detail_transaksi ON menu.id_menu = detail_transaksi.id_menu GROUP BY menu.id_menu LIMIT 5",
    (err, results, fields) => {
      if (!err) {
        res.send(results);
      } else {
...
app.get("/avg", async (req, res) => {
  conn.query(
    "SELECT menu.id_menu, menu.nama_menu, MIN(detail_transaksi.qty) AS total_qty FROM menu JOIN detail_transaksi ON menu.id_menu = detail_transaksi.id_menu GROUP BY menu.id_menu LIMIT 5",
    (err, results, fields) => {
      if (!err) {
        res.send(results);
      } else {
...
app.get("/wilayah/:alamat_pelanggan", auth, async (req, res) => {
  let param = {
    alamat_pelanggan: req.params.alamat_pelanggan,
  };
  transaksi
    .findAll({
      include: ["user", "meja"],
        ...
        [Op.or]: [
          {
            "$transaksi.alamat_pelanggan$": {
              [Op.like]: `%${param.alamat_pelanggan}%`,
            },
          },
        ],
      },
